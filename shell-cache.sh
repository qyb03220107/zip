...
IPADDR=$(ifconfig  ens33|grep   inet|grep -v inet6 |awk '{print $2}')
...
RESULTFILE="$LOGPATH/$HOSTNAME-$IPADDR-`date +%Y%m%d%H%M%S`.txt"
...
function getDiskStatus(){
    ...
    #报表信息
    diskdata=$(df -HT | sed '1d' | awk '$2!="tmpfs"{print}') #KB
    disktotal=$(echo "$diskdata" | awk '{total+=$3}END{print total}') #KB
    diskused=$(echo "$diskdata" | awk '{total+=$4}END{print total}')  #KB
    diskfree=$(echo "$disktotal-$diskused"|bc) #KB
#    c=$(echo "$a-$b"|bc)
    diskusedpercent=$(echo $disktotal $diskused | awk '{if($1==0){printf 100}else{printf "%.2f",$2*100/$1}}') 
    inodedata=$(df -HT | sed '1d' | awk '$2!="tmpfs"{print}')
    inodetotal=$(echo "$inodedata" | awk '{total+=$3}END{print total}')
    inodeused=$(echo "$inodedata" | awk '{total+=$4}END{print total}')
    inodefree=$(echo "$inodetotal-$inodeused"|bc)
    inodeusedpercent=$(echo $inodetotal $inodeused | awk '{if($1==0){printf 100}else{printf "%.2f",$2*100/$1}}')
    report_DiskTotal=$(echo "$disktotal/1024/1024"|bc)"GB"   #硬盘总容量(GB)
    report_DiskFree=$(echo "$diskfree/1024/1024"|bc)"GB"     #硬盘剩余(GB)
    report_DiskUsedPercent="$diskusedpercent""%"    #硬盘使用率%
    report_InodeTotal=$(echo "$inodetotal/1000"|bc)"K"       #Inode总量
    report_InodeFree=$(echo "$inodefree/1000"|bc)"K"         #Inode剩余
